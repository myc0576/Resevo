...
项目品牌名和当前 canonical 本地入口都是 Resevo：`G:\BaiduSyncdisk\Resevo`。历史名称 `research-harness` 只作为旧路径或兼容性标识出现，不再作为推荐工作目录。
...
项目素材写入当前项目的 `research_assets\YYYYMMDD_task_name`。Resevo 规则、skills、templates、reports、registries、decisions 只写入 `G:\BaiduSyncdisk\Resevo`。
...
```powershell
python G:\BaiduSyncdisk\Resevo\scripts\self_evolution_loop.py recall --query "<当前任务>" --project-root <当前项目路径> --json
python G:\BaiduSyncdisk\Resevo\scripts\self_evolution_loop.py init --project-root <当前项目路径> --trigger "自进化" --out <intake.yaml>
python G:\BaiduSyncdisk\Resevo\scripts\self_evolution_loop.py run --intake <intake.yaml> --apply-candidates --json
python G:\BaiduSyncdisk\Resevo\scripts\self_evolution_loop.py resume --run-id <run_id> --json
```
...
Resevo 适用于整个 `G:\` 科研工作区，不只适用于 `G:\projects\SC-NMT`。
