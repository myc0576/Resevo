# Reference Projects and Resevo Boundaries

This document records architectural borrowing boundaries. Resevo does not copy
source code from these projects. It keeps a local-first, agent-agnostic scope:
an external agent performs research while Resevo governs workflow memory,
...
- **Resevo boundary:** OpenWiki manages what a codebase or project knows. Resevo
  manages how a scientific task should be done next, including claim, evidence,
  artifact, decision, and validation gates. Resevo does not become a generic
...
- **Resevo implementation:** `resevo init`, portable workspace paths, SQLite FTS
...
- **Resevo boundary:** Resevo does not implement or claim reinforcement learning.
...
- **License and implementation risk:** The repository is MIT licensed. It is
  ...
  services; importing that runtime would violate Resevo's local-first boundary.
- **Resevo implementation:** `semantic_relevance`, `domain_fit`,
  `evidence_grade`, reuse counts, freshness, feedback, provenance, and
  `utility_score` are stored or derived in `resevo.retrieval`.
...
- **Resevo boundary:** Resevo evolves research workflows, not a universal memory
...
- **License and implementation risk:** SimpleMem is MIT licensed. Its hybrid
  ...
  runtime surface.
- **Resevo implementation:** `resevo.evolution.evaluate_guard` persists
...
- **Resevo boundary:** Open Science is an autonomous research application with
  an agent and integrations. Resevo does not bundle an Agent, model, provider,
...
- **License and implementation risk:** The repository is MIT licensed. Its beta
  ...
  small Python CLI/MCP engine and should not be vendored into Resevo.
- **Resevo implementation:** `run.json`, `trace.jsonl`,
  ...
  `.resevo/runs/` without copying source artifacts.
...
- **Borrowable capability:** A product-shaped local CLI plus MCP installation
  ...
  command delegation.
- **Resevo boundary:** OpenDesign is a design workspace and skill system.
  Resevo does not copy its templates, skills, UI, or agent-specific product
...
- **License and implementation risk:** The core repository is Apache-2.0, while
  ...
  configuration formats can change, so Resevo must prefer official `mcp add`,
  `mcp get`, and `mcp remove` commands and print a snippet when uncertain.
- **Resevo implementation:** `resevo mcp install|status|uninstall codex|claude`
...
| Project | Primary object | Resevo difference |
|---|---|---|
...
| Resevo | Evidence-governed research workflow harness | CLI + stdio MCP external brain for Codex/Claude Code |
