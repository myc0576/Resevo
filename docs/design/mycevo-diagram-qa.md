...
The four final files are native 1600×900 SVGs with editable text and vector primitives. Light and dark variants have identical labels, object structure, and relationships; only theme tokens change.
...
- `50.png`: 800×450.
- `25.png`: 400×225.
- `github.png`: 840×473 representative README width.

At 100%, 50%, and GitHub width, titles, stage labels, boundaries, and gate paths are clear. At 25%, the main sequence, candidate/gate/reusable distinction, feedback loop, public/private boundary, and arrow directions remain understandable. Microcopy is intentionally secondary and is not reliably readable at 25%; the SVG text remains available when opened directly.

No reviewed render contains clipped headings, overlapping stage labels, transparent canvas gaps, or line weights that disappear at 25%.
...
- Light/dark product files each contain 21 text nodes; light/dark technical files each contain 37.
...
- File sizes remain small: roughly 7.4 KB for each product SVG and 9.9 KB for each technical SVG.
...
- Microcopy is not intended to be read at 400×225; primary labels and structural meaning are the 25% target.
- The final diagrams are English-only. The Chinese README uses the same diagrams with Chinese surrounding explanation to avoid introducing a lower-quality unverified translation variant.
